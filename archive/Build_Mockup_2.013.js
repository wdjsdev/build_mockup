...
function container()
{
	...
	/////////////////////////////
		...
		function getStyleNum(topbot)
		{
			...
			try
			{
				garment.styleNum = json[topbot].styleNo;
				if(garment.styleNum != undefined)
				{
					log.l("Successfully set garment.styleNum to " + garment.styleNum);
				}
				else
				{
					localValid = false
					errorList.push("Style number was returned undefined.");
					log.e(""))
				}
			}
			catch(e)
	...
	//masterLoop Function Description
	//loop the garmentsNeeded array and get the garment info for each garment
	function masterLoop(json)
	{
		var localValid = true;
		...
		for(var ml=0;ml<garmentsNeeded.length;ml++)
		{
			...
			//check if garment is top or bottom
			if(thisGarment == "top")
			{
				//garment is top, find garment info for tops
			...
			else if(thisGarment == "bot")
			{
				//garment is bottom, find garment info for bottoms
			}
			else
			{
				errorList.push("The garment information was not properly stored. The script does not know what kind of garment this is..")
			}
		}
	...
	//////////////////////////////////////////////////////////////////////
		...
		else if(valid)
		{
			log.h("json data has been read and validated.::Beginning master loop to find info " +
					"for each garment.");
			if(masterLoop(lib.json))
			{
				log.l("masterLoop function returned true. All garment info has been successfully obtained.");
			}
			else
			{
				valid = false;
