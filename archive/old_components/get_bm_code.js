/*
	Component Name: get_bm_code
	Author: William Dowling
	Creation Date: 08 March, 2017
	Description: 
		take the garment name and find the appropriate bm code from the midtable
	Arguments
		garments object
			{
				top: {name: "FD-SLOW-SS",styleNum:"1013"},
				bottom: {name: "PS-5014",styleNum:"1002"}
			}
	Return value
		object
			{
				top: "FD-161",
				bottom: "PS-5014"
			}

*/

//getGarmentCode Function Description
//search midtable for a bm code that matches the garment name
//check whether the bm code is style number specific 
	//(i.e. slowpitch could be FD-161 or FD-163 depending upon style number.)
function getGarmentCode(garObj)
{
	var result = {top:"",bottom:""};
	var garmentCode;

	log.h("Beginning execution of getGarmentCode function.");

	var midTable = 
	{
		'BA-APPAREL-HD-BM868': 				 	'BM-868'
		, 'BA-APPAREL-HD-BM868W': 				'BM-868W'
		, 'BA-APPAREL-HD-BM869': 				'BM-869'
		, 'BA-APPAREL-HD-BM869W': 				'BM-869W'
		, 'BA-APPAREL-LS-BM162': 				'BM-162'
		, 'BA-APPAREL-LS-BM162W': 				'BM-162W'
		, 'BA-APPAREL-LS-BM3001': 				'BM-3001'
		, 'BA-APPAREL-LS-BM3001W': 				'BM-3001W'
		, 'BA-APPAREL-LS-BM3002': 				'BM-3002'
		, 'BA-APPAREL-LSPO-BM3100': 			'BM-3100'
		, 'BA-APPAREL-LSPO-BM3100W': 			'BM-3100W'
		, 'BA-APPAREL-LSPO-BM487': 				'BM-487'
		, 'BA-APPAREL-LSQZ-BM477': 				'BM-477'
		, 'BA-APPAREL-LSQZ-BM682': 				'BM-682'
		, 'BA-APPAREL-LSQZ-BM682W': 			'BM-682W'
		, 'BA-APPAREL-SL-BM342': 				'BM-342'
		, 'BA-APPAREL-SS-BM163': 				'BM-163'
		, 'BA-APPAREL-SS-BM3085': 				'BM-3085'
		, 'BA-APPAREL-SS-BM340': 				'BM-340'
		, 'BA-APPAREL-SS-BM345': 				'BM-345'
		, 'BA-APPAREL-SS-BM345W': 				'BM-345W'
		, 'BA-APPAREL-SS-BM346': 				'BM-346'
		, 'BA-APPAREL-SS-BM807': 				'BM-807'
		, 'BA-APPAREL-SS-PS345': 				'BM-345'
		, 'BA-APPAREL-SS-PS807': 				'BM-807'
		, 'BA-APPAREL-SSQZ-BM475': 				'BM-475'
		, 'BA-APPAREL-SSQZ-BM611': 				'BM-611'
		, 'BA-BASE-2BSS-BM235': 				'BM-235'
		, 'BA-BASE-2BSS-BM236': 				'BM-236'
		, 'BA-BASE-2BSS-BM3406': 			 	'BM-3406'
		, 'BA-BASE-2BSS-BM3411': 			 	'BM-3411'
		, 'BA-BASE-2BSS-BM3412': 				'BM-3412'
		, 'BA-BASE-2BSS-BM3416': 				'BM-3416'
		, 'BA-BASE-2BSS-BM3418': 			 	'BM-3418'
		, 'BA-BASE-2BSS-BM3419': 				'BM-3419'
		, 'BA-BASE-FBSL-BM3401': 				'BM-3401'
		, 'BA-BASE-FBSL-BM3402': 				'BM-3402'
		, 'BA-BASE-FBSL-BM3415': 				'BM-3415'
		, 'BA-BASE-FBSS-BM3403': 				'BM-3403'
		, 'BA-BASE-FBSS-BM3404': 				'BM-3404'
		, 'BA-BASE-FBSS-BM3405': 				'BM-3405'
		, 'BA-BASE-FBSS-BM3408': 				'BM-3408'
		, 'BA-BASE-FBSS-BM3409': 				'BM-3409'
		, 'BA-BASE-FBSS-BM3410': 				'BM-3410'
		, 'BA-BASE-FBSS-BM3413': 				'BM-3413'
		, 'BA-BASE-FBSS-BM3414': 				'BM-3414'
		, 'BA-BASE-SS-BM3407': 			 		'BM-3407'
		, 'BA-BASE-SS-BM3417': 					'BM-3417'
		, 'BA-FAST-2BSS': 						'BM-4404W'
		, 'BA-FAST-2BSS-BM4404W': 				'BM-4404W'
		, 'BA-FAST-2BSS-BM4405W': 				'BM-4405W'
		, 'BA-FAST-2BSS-BM4411W': 				'BM-4411W'
		, 'BA-FAST-2BSS-BM4412W': 			 	'BM-4412W'
		, 'BA-FAST-2BSS-BM4415W': 			 	'BM-4415W'
		, 'BA-FAST-FBSL-BM4401W': 			 	'BM-4401W'
		, 'BA-FAST-FBSL-BM4402W': 			 	'BM-4402W'
		, 'BA-FAST-FBSL-BM4414W': 			 	'BM-4414W'
		, 'BA-FAST-FBSS-BM4403W': 			 	'BM-4403W'
		, 'BA-FAST-FBSS-BM4408W': 			 	'BM-4408W'
		, 'BA-FAST-FBSS-BM4409W': 			 	'BM-4409W'
		, 'BA-FAST-FBSS-BM4410W': 			 	'BM-4410W'
		, 'BA-FAST-FBSS-BM4413W': 			 	'BM-4413W'
		, 'BA-FAST-SLVN-BM4406W': 			 	'BM-4406W'
		, 'BA-FAST-SS-BM4407W': 			 	'BM-4407W'
		, 'FD-BASE-2BSS': 			 			'FD-234'
		, 'FD-BASE-2BSS-HT': 			 		'FD-234'
		, 'FD-BASE-2BSS-LD': 			 		'FD-234'
		, 'FD-BASE-2BSS-MMP': 			 		'FD-234'
		, 'FD-BASE-FBSL': 			 			'FD-233'
		, 'FD-BASE-FBSL-HT': 			 		'FD-233'
		, 'FD-BASE-FBSL-LD': 			 		'FD-233'
		, 'FD-BASE-FBSL-MMP': 			 		'FD-233'
		, 'FD-BASE-FBSS': 			 			'FD-230'
		, 'FD-BASE-FBSS-HT': 			 		'FD-230'
		, 'FD-BASE-FBSS-LD': 			 		'FD-230'
		, 'FD-BASE-FBSS-MMP': 			 		'FD-230'
		, 'FD-BASE-SS': 			 			'FD-163/FD-161'
		, 'FD-BASE-SS-DE': 			 			'FD-163/FD-161'
		, 'FD-BASE-SS-MM': 			 			'FD-163/FD-161'
		, 'FD-BASE-VN': 			 			'FD-3417'
		, 'FD-BASK-LS': 			 			'FD-622'
		, 'FD-BASK-SH': 			 			'FD-211'
		, 'FD-BASK-SHRV2P': 					'FD-215'
		, 'FD-BASK-SL': 			 			'FD-210'
		, 'FD-BASK-SLRV2P': 			 		'FD-219'
		, 'FD-BASK-SS': 			 			'FD-137'
		, 'FD-BASKW-LS': 			 			'FD-622W'
		, 'FD-BASKW-SH': 			 			'FD-221W'
		, 'FD-BASKW-SHRV2P': 				 	'FD-215W'
		, 'FD-BASKW-SL': 			 			'FD-220W'
		, 'FD-BASKW-SLRV2P': 			 		'FD-219W'
		, 'FD-BASKW-SS': 			 			'FD-137W'
		, 'FD-FAST-2BSS': 			 			'FD-246W'
		, 'FD-FAST-2BSS-HT': 			 		'FD-246W'
		, 'FD-FAST-2BSS-LD': 			 		'FD-246W'
		, 'FD-FAST-2BSS-MMP': 			 		'FD-246W'
		, 'FD-FAST-FBSL': 			 			'FD-243W'
		, 'FD-FAST-FBSL-HT': 				 	'FD-243W'
		, 'FD-FAST-FBSL-LD': 			 		'FD-243W'
		, 'FD-FAST-FBSL-MMP': 			 		'FD-243W'
		, 'FD-FAST-FBSS': 			 			'FD-240W'
		, 'FD-FAST-FBSS-HT': 			 		'FD-240W'
		, 'FD-FAST-FBSS-LD': 			 		'FD-240W'
		, 'FD-FAST-FBSS-MMP': 			 		'FD-240W'
		, 'FD-FAST-RB': 			 			'FD-400W'
		, 'FD-FAST-RB-DE': 			 			'FD-400W'
		, 'FD-FAST-SH-DE': 			 			'FD-400W'
		, 'FD-FAST-SL': 			 			'FD-500W'
		, 'FD-FAST-SL-DE': 			 			'FD-500W'
		, 'FD-FAST-SS': 			 			'FD-163W/FD-161W'
		, 'FD-FAST-SS-DE': 			 			'FD-163W/FD-161W'
		, 'FD-FAST-SS-MM': 			 			'FD-163W/FD-161W'
		, 'FD-FAST-VN': 			 			'FD-4416W'
		, 'FD-FOOT-PANT': 			 			'FD-251'
		, 'FD-FOOT-PNLPNT': 			 		'FD-5080'
		, 'FD-FOOT-SS': 			 			'FD-250'
		, 'FD-FOOT-SSBASIC': 			 		'FD-5411'
		, 'FD-FOOTY-PANT': 			 			'FD-251Y'
		, 'FD-LAX-PN2PRV': 			 			'FD-3027/FD-3024Y'
		, 'FD-LAX-PN2PRVPJ': 			 		'FD-3042/FD-3038Y'
		, 'FD-LAX-PNRV': 			 			'FD-2020'
		, 'FD-LAX-RSS': 			 			'FD-260/FD-3007'
		, 'FD-LAX-SH': 			 				'FD-261'
		, 'FD-LAX-SLRV': 			 			'FD-2000'
		, 'FD-LAX-SS': 			 				'FD-260/FD-3007'
		, 'FD-LAX-SSRV': 			 			'FD-3050'
		, 'FD-LAXW-KT': 			 			'FD-4014W'
		, 'FD-LAXW-PN2PRV': 			 		'FD-3019W'
		, 'FD-LAXW-RBVN': 			 			'FD-3015W'
		, 'FD-LAXW-SH': 			 			'FD-4004W'
		, 'FD-LAXW-SLVN': 			 			'FD-3045W'
		, 'FD-LAXW-SS': 			 			'FD-3011W'
		, 'FD-SLOW-LS-DE': 			 			'FD-164'
		, 'FD-SLOW-LS-MM': 			 			'FD-164'
		, 'FD-SLOW-SS': 			 			'FD-163/FD-161'
		, 'FD-SLOW': 							'FD-163/FD-161'
		, 'FD-SLOW-SS-DE': 			 			'FD-163/FD-161'
		, 'FD-SLOW-SS-MM': 			 			'FD-163/FD-161'
		, 'FD-SLOW-VN': 			 			'FD-3417'
		, 'FD-SLOWW-SS-DE': 			 		'FD-163W/FD-161W'
		, 'FD-SLOWW-SS-MM': 			 		'FD-163W/FD-161W'
		, 'FD-SOC-LSK': 			 			'FD-3092'
		, 'FD-SOC-SH': 			 				'FD-857'
		, 'FD-SOC-SS': 			 				'FD-3062'
		, 'FD-SOC-WU': 							'FD-6062'
		, 'FD-SP-BKSWP': 			 			'FD-211'
		, 'FD-SP-FCN': 			 				'FD-486'
		, 'FD-SP-HD': 			 				'FD-692/FD-872'
		, 'FD-SP-LIGHTHD': 			 			'FD-1873'
		, 'FD-SP-LIGHTQZ': 			 			'FD-682'
		, 'FD-SP-LS': 			 				'FD-164'
		, 'FD-SP-LSQZPO': 			 			'FD-7025'
		, 'FD-SP-POLO': 			 			'FD-597'
		, 'FD-SP-SH4PBP': 			 			'FD-634'
		, 'FD-SP-SS': 				 			'FD-161/FD-163'
		, 'FD-SP-SSCOMP': 			 			'FD-410'
		, 'FD-SP-USP': 			 				'FD-648'
		, 'FD-SP-VERGECN': 					 	'FD-487'
		, 'FD-SP-VERGEQZ': 			 			'FD-6061'
		, 'FD-SP-WLSPO': 			 			'FD-477'
		, 'FD-SP-WSSPO': 			 			'FD-611'
		, 'FD-SP-WU': 			 				'FD-6062'
		, 'FD-SP-WUP': 			 				'FD-6063'
		, 'FD-SPIRIT-BKSWP-CM': 			 	'FD-211'
		, 'FD-SPIRIT-HD-FL': 			 		'FD-872/FD-692'
		, 'FD-SPIRIT-SS-DE': 			 		'FD-163/FD-161'
		, 'FD-SPIRIT-USP-CM': 			 		'FD-648'
		, 'FD-SPW-CAP': 			 			'FD-5029W'
		, 'FD-SPW-CC': 			 				'FD-3069W'
		, 'FD-SPW-FCN': 			 			'FD-486W'
		, 'FD-SPW-HD': 			 				'FD-872W/FD-692W'
		, 'FD-SPW-LEG': 			 			'FD-5036W'
		, 'FD-SPW-LIGHTHD': 			 		'FD-1873W'
		, 'FD-SPW-LSQZPO': 			 			'FD-7020W'
		, 'FD-SPW-POLO': 			 			'FD-597W'
		, 'FD-SPW-SB': 			 				'FD-762W'
		, 'FD-SPW-SS': 			 				'FD-163W/FD-161W'
		, 'FD-SPW-TSH': 			 			'FD-4018W'
		, 'FD-SPW-WQZ': 			 			'FD-6002W'
		, 'FD-SPW-WU': 			 				'FD-6062W'
		, 'FD-SPW-WUP': 			 			'FD-6063W'
		, 'FD-VOL-CS': 			 				'FD-3184W'
		, 'FD-VOL-LS': 			 				'FD-3185W'
		, 'FD-VOL-SH': 			 				'FD-281'
		, 'FD-VOL-WU': 			 				'FD-3003W'
		, 'FD-WSPIRIT-HD-FL': 			 		'FD-872W/FD-692W'
		, 'FD-WSPIRIT-SS-DE': 			 		'FD-163W/FD-161W'
		, 'BA-PANTW-HSERIES-BM5093W': 			'BM-5093W'
		, 'BA-PANTW-HSERIES-BM5088W': 			'BM-5088W'
		, 'BA-PANTW-HSERIES-BM5089W': 			'BM-5089W'
		, 'BA-PANTW-HSERIES-BM5091W': 			'BM-5091W'
		, 'BA-PANTW-HSERIES-BM5087W': 			'BM-5087W'
		, 'BA-PANTW-HSERIES-BM5084W': 			'BM-5084W'
		, 'BA-PANTW-HSERIES-BM5085W': 			'BM-5085W'
		, 'BA-PANTW-HSERIES-BM5092W': 			'BM-5092W'
		, 'BA-PANTW-HSERIES-BM5082W': 			'BM-5082W'
		, 'BA-PANTW-HSERIES-BM5090W': 			'BM-5090W'
		, 'BA-PANTW-HSERIES-BM5060W': 			'BM-5060W'
		, 'BA-PANTW-HSERIES-BM5070W': 			'BM-5070W'
		, 'BA-PANTW-HSERIES-BM5095W': 			'BM-5095W'
		, 'BA-PANTW-HSERIES-BM5039W': 			'BM-5039W'
		, 'BA-PANTW-HSERIES-BM5041W': 			'BM-5041W'
		, 'BA-PANTW-HSERIES-BM5050W': 			'BM-5050W'
		, 'BA-PANT-PSY-PS5014': 				'PS-5014Y'
		, 'BA-PANT-PS-PS5014': 					'PS-5014'
		, 'FD-PANTW-PSHTS': 			 		'PS-5075'
		, 'FD-PANTW-FDHTS': 			 		'FD-5060W'
		, 'FD-SP-LIGHTQZ': 			 			'FD-682'
		, 'FD-SP-VERGEQZ': 			 			'FD-6061'
		, 'FD-SOCW-SS': 			 			'FD-3047W/FD-3048W'
		, 'FD-SOCW-LSK': 			 			'FD-3090W'
		, 'FD-SOCW-SH': 			 			'FD-4005W'
		, 'FD-SP-BGTG': 			 			'FD-106'
		, 'BA-APPAREL-WUJ-BM6017': 			 	'BM-6017'
		, 'BA-APPAREL-WUJ-BM6017W': 			'BM-6017W'
		, 'BA-APPAREL-WUP-BM6018': 			 	'BM-6018'
		, 'BA-APPAREL-WUP-BM6018W': 			'BM-6018W'
		, 'BA-APPAREL-SH-BM4025': 			 	'BM-4025'
		, 'BA-APPAREL-SH-BM4026': 			 	'BM-4026'
		, 'BA-APPAREL-SH-BM4027': 			 	'BM-4027'
		, 'BA-APPAREL-SH-BM4028': 			 	'BM-4028'
		, 'BA-APPAREL-SH-BM4029': 			 	'BM-4029'
		, 'BA-APPAREL-SH-BM4030': 			 	'BM-4030'
		, 'BA-APPAREL-SH-BM659': 			 	'BM-659'
		, 'BA-APPAREL-LSPO-BM3099W': 			'BM-3099W'
		, 'BA-APPAREL-SS-BM340': 			 	'BM-340'
		, 'BA-APPAREL-LSPO-BM487W': 			'BM-487W'
		, 'BA-APPAREL-LSQZ-BM6061W': 			'BM-6061W'
		, 'FD-SPW-WSSQZ': 			 			'FD-6004'
		, 'FD-SLOW-SL': 			 			'FD-609'
		, 'FD-BASE-SLCN': 			 			'FD-609'
		, 'FD-FAST-SLCN': 			 			'FD-609W'
		, 'Doesn\'t Exist':						'FD-999/FD-999Y'
	}


	for(var topBot in garObj)
	{
		if(!garObj[topBot])
		{
			log.l("garObj." + topBot + " is null.");
			continue;
		}
		log.h("Getting the garmentCode for " + topBot);
		var name = garObj[topBot].name
		var styleNum = garObj[topBot].styleNum;
		//get the garmentCode from midtable
		if(midTable[name])
		{
			garmentCode = midTable[name]
			log.l("The garmentCode for " + name + " in the midtable was returned as " + garmentCode);

			//check whether there is a forward slash in garmentCode
			//if forward slash exists, we need to find the substyle
			//example: FD-161/FD-163
			if(garmentCode.indexOf("/")>-1)
			{
				log.l("garmentCode: " + garmentCode + ": contains a \"/\". getSubStyle function necessary to determine correct garmentCode.")
				garmentCode = getSubStyle(name,styleNum);
			}

			//check whether getSubStyle function was successful
			if(garmentCode == undefined)
			{
				log.e("The garmentCode was found in the midtable and contained multiple codes.::However the getSubStyle function returned undefined.\
					::Check the getSubStyle function as it pertains to " + midTable[name]);
				errorList.push("Sorry, the BM Code was not properly determined.");
			}

			result[topBot] = garmentCode;

		}
		else
		{
			log.e("There is no entry in the midtable for " + name + "::Prompting the user to enter the correct BM code.");
			errorList.push("Sorry. It looks like there was an issue automatically determining the correct BM code for " + name);
			errorList.push("Thanks for your patience while this issue is resolved.");
			

			garmentCode = promptForgarmentCode(name);
			if(garmentCode == undefined)
			{
				log.e("User cancelled dialog.");
				errorList.push("Sorry. Could not determine the correct BM Code for " + name);
				result[topBot] = undefined;
			}
			else
			{
				result[topBot] = garmentCode;
			}
		}
	}



	function getSubStyle(itemName, style)
	{
		var subStyle;

		log.l("Beginning getSubStyle function with arguments: ::itemName = " + itemName + "::style = " + style);

		var fdSlowSS = {"1000":"FD-163", "1001":"FD-163", "1002":"FD-163", "1003":"FD-163", "1005":"FD-163", "1006":"FD-163", "1007":"FD-163", "1009":"FD-163", "1010":"FD-163", "1011":"FD-163", "1012":"FD-163", "1013":"FD-163", "1014":"FD-163", "1015":"FD-163", "1017":"FD-163", "1018":"FD-163", "1019":"FD-163", "1020":"FD-163", "1022":"FD-163", "1023":"FD-163", "1024":"FD-163", "1025":"FD-163", "1030":"FD-163", "1035":"FD-163", "1036":"FD-163", "1037":"FD-163", "1040":"FD-163", "1041":"FD-163", "1042":"FD-163", "1043":"FD-163", "1044":"FD-163", "1045":"FD-163", "1046":"FD-163", "1047":"FD-163", "1048":"FD-163", "1049":"FD-163", "1052":"FD-163", "1053":"FD-163", "1054":"FD-163", "1056":"FD-163", "1059":"FD-163", "1062":"FD-163", "1063":"FD-163", "1064":"FD-163", "1065":"FD-163", "1066":"FD-163", "1067":"FD-163", "1068":"FD-163","1004":"FD-161", "1008":"FD-161", "1016":"FD-161", "1021":"FD-161", "1026":"FD-161", "1029":"FD-161", "1031":"FD-161", "1032":"FD-161", "1033":"FD-161", "1034":"FD-161", "1038":"FD-161", "1039":"FD-161", "1050":"FD-161", "1055":"FD-161", "1057":"FD-161", "1058":"FD-161", "1060":"FD-161", "1061":"FD-161", "1069":"FD-161", "1070":"FD-161", "1071":"FD-161"};
		var fdFastSS = {"1000"	:"FD-163W", "1001":"FD-163W", "1002":"FD-163W", "1003":"FD-163W", "1005":"FD-163W", "1006":"FD-163W", "1007":"FD-163W", "1009":"FD-163W", "1010":"FD-163W", "1011":"FD-163W", "1012":"FD-163W", "1013":"FD-163W", "1014":"FD-163W", "1015":"FD-163W", "1017":"FD-163W", "1018":"FD-163W", "1019":"FD-163W", "1020":"FD-163W", "1022":"FD-163W", "1023":"FD-163W", "1024":"FD-163W", "1025":"FD-163W", "1030":"FD-163W", "1035":"FD-163W", "1036":"FD-163W", "1037":"FD-163W", "1040":"FD-163W", "1041":"FD-163W", "1042":"FD-163W", "1043":"FD-163W", "1044":"FD-163W", "1045":"FD-163W", "1046":"FD-163W", "1047":"FD-163W", "1048":"FD-163W", "1049":"FD-163W", "1052":"FD-163W", "1053":"FD-163W", "1054":"FD-163W", "1056":"FD-163W", "1059":"FD-163W", "1062":"FD-163W", "1063":"FD-163W", "1064":"FD-163W", "1065":"FD-163W", "1066":"FD-163W", "1067":"FD-163W", "1068":"FD-163W","1004":"FD-161W", "1008":"FD-161W", "1016":"FD-161W", "1021":"FD-161W", "1026":"FD-161W", "1029":"FD-161W", "1031":"FD-161W", "1032":"FD-161W", "1033":"FD-161W", "1034":"FD-161W", "1038":"FD-161W", "1039":"FD-161W", "1050":"FD-161W", "1055":"FD-161W", "1057":"FD-161W", "1058":"FD-161W", "1060":"FD-161W", "1061":"FD-161W", "1069":"FD-161W", "1070":"FD-161W", "1071":"FD-161W"};
		var fdSlowySS = {"1000":"FD-163Y", "1001":"FD-163Y", "1002":"FD-163Y", "1003":"FD-163Y", "1005":"FD-163Y", "1006":"FD-163Y", "1007":"FD-163Y", "1009":"FD-163Y", "1010":"FD-163Y", "1011":"FD-163Y", "1012":"FD-163Y", "1013":"FD-163Y", "1014":"FD-163Y", "1015":"FD-163Y", "1017":"FD-163Y", "1018":"FD-163Y", "1019":"FD-163Y", "1020":"FD-163Y", "1022":"FD-163Y", "1023":"FD-163Y", "1024":"FD-163Y", "1025":"FD-163Y", "1030":"FD-163Y", "1035":"FD-163Y", "1036":"FD-163Y", "1037":"FD-163Y", "1040":"FD-163Y", "1041":"FD-163Y", "1042":"FD-163Y", "1043":"FD-163Y", "1044":"FD-163Y", "1045":"FD-163Y", "1046":"FD-163Y", "1047":"FD-163Y", "1048":"FD-163Y", "1049":"FD-163Y", "1052":"FD-163Y", "1053":"FD-163Y", "1054":"FD-163Y", "1056":"FD-163Y", "1059":"FD-163Y", "1062":"FD-163Y", "1063":"FD-163Y", "1064":"FD-163Y", "1065":"FD-163Y", "1066":"FD-163Y", "1067":"FD-163Y", "1068":"FD-163Y","1004":"FD-161Y", "1008":"FD-161Y", "1016":"FD-161Y", "1021":"FD-161Y", "1026":"FD-161Y", "1029":"FD-161Y", "1031":"FD-161Y", "1032":"FD-161Y", "1033":"FD-161Y", "1034":"FD-161Y", "1038":"FD-161Y", "1039":"FD-161Y", "1050":"FD-161Y", "1055":"FD-161Y", "1057":"FD-161Y", "1058":"FD-161Y", "1060":"FD-161Y", "1061":"FD-161Y", "1069":"FD-161Y", "1070":"FD-161Y", "1071":"FD-161Y"};
		var fdLaxRSS = {"1000":"FD-260","1001":"FD-260","1002":"FD-260","1003":"FD-260","1004":"FD-260","1005":"FD-3007","1006":"FD-3007","1007":"FD-3007","1008":"FD-3007","1017":"FD-260","1018":"FD-260","1019":"FD-260","1020":"FD-3007","1021":"FD-3007","1022":"FD-3007"}
		var fdLaxSS = {"1000":"FD-260","1001":"FD-260","1002":"FD-260","1003":"FD-260","1004":"FD-260","1005":"FD-3007","1006":"FD-3007","1007":"FD-3007","1008":"FD-3007","1017":"FD-260","1018":"FD-260","1019":"FD-260","1020":"FD-3007","1021":"FD-3007","1022":"FD-3007"};	
		var fdLaxWSS = {"1000":"FD-3011W","1001":"FD-3047W","1002":"FD-3047W","1003":"FD-3047W","1004":"FD-3047W","1005":"FD-3047W","1006":"FD-3047W","1007":"FD-3047W","1008":"FD-3047W","1009":"FD-3011W","1010":"FD-3011W","1011":"FD-3011W","1012":"FD-3011W","1013":"FD-3011W","1014":"FD-3011W","1015":"FD-3011W","1016":"FD-3011W","1017":"FD-3011W"};
		var fdSpSS = {"1000":"FD-163","1003":"FD-163","1004":"FD-139","1007":"FD-163","1008":"FD-139","1012":"FD-163","1015":"FD-163","1043":"FD-163","1065":"FD-163","1066":"FD-163"};
		var fdSpHD = {"1000":"FD-872","1001":"FD-692","1002":"FD-692","1003":"FD-692","1004":"FD-692","1006":"FD-872","1007":"FD-872","1008":"FD-872","1009":"FD-872","1010":"FD-872","1011":"FD-872","1012":"FD-872","1013":"FD-872","1014":"FD-872","1015":"FD-872","1016":"FD-872","1017":"FD-872","1018":"FD-872","1019":"FD-872","1022":"FD-872"};
		var fdSpwSS = {"1001":"FD-163W","1003":"FD-163W","1004":"FD-139W","1007":"FD-163W","1008":"FD-139W","1012":"FD-163W","1015":"FD-163W","1043":"FD-163W","1065":"FD-163W","1066":"FD-163W"};
		var fdSpwHD = {"1000":"FD-872W","1001":"FD-692W","1002":"FD-692W","1003":"FD-692W","1004":"FD-692W","1006":"FD-872W","1007":"FD-872W","1008":"FD-872W","1009":"FD-872W","1010":"FD-872W","1011":"FD-872W","1012":"FD-872W","1013":"FD-872W","1014":"FD-872W","1015":"FD-872W","1016":"FD-872W","1017":"FD-872W","1018":"FD-872W","1019":"FD-872W","1022":"FD-872W"};
		var fdFastSL = {"1000":"FD-500W","1001":"FD-500W","1002":"FD-500W","1003":"FD-500W","1004":"FD-500W","1005":"FD-500W","1006":"FD-500W","1007":"FD-500W","1008":"FD-500W","1009":"FD-500W","1010":"FD-500W","1011":"FD-500W","1012":"FD-500W","1013":"FD-500W","1014":"FD-500W","1015":"FD-500W","1016":"FD-500W","1017":"FD-500W","1018":"FD-500W","1019":"FD-500W","1020":"FD-500W","1021":"FD-500W","1022":"FD-500W","1023":"FD-500W","1024":"FD-500W","1025":"FD-500W","1026":"FD-500W","1027":"FD-500W","1052":"FD-500W","1053":"FD-500W","1054":"FD-500W","1055":"FD-500W","1057":"FD-500W","1058":"FD-500W","1059":"FD-500W","1060":"FD-500W","1061":"FD-500W","1062":"FD-500W","1062":"FD-500W","1063":"FD-500W","1064":"FD-500W","1065":"FD-500W","1066":"FD-500W","1067":"FD-500W"}
		var fdSocSS = {"1000":"FD-858","1001":"FD-858","1002":"FD-858","1003":"FD-858","1004":"FD-858","1005":"FD-3061","1006":"FD-3061","1007":"FD-3061","1008":"FD-3062","1009":"FD-3062","1010":"FD-3062","1011":"FD-3063","1012":"FD-3063","1013":"FD-3063","1014":"FD-3063","1015":"FD-3063","1016":"FD-3064","1017":"FD-3064","1018":"FD-3064","1019":"FD-3064","1020":"FD-3064"};
		var fdSocWomensSS = {"1000":"FD-3037W","1001":"FD-3037W","1002":"FD-3037W","1003":"FD-3037W","1004":"FD-3048W","1005":"FD-3048W","1006":"FD-3048W","1007":"FD-3048W"};


		switch(itemName)
		{
			case 'FD-SLOW-SS':
			case 'FD-SLOW-SS-DE':
			case 'FD-SLOW-SS-MM':
			case 'FD-BASE-SS':
				subStyle = fdSlowSS[style];
				break;
			case 'FD-BASE-SS':
				subStyle = fdBaseSS[style];
				break;
			case 'FD-FAST':
				subStyle = fdSlowwSS[style];
				break;
			case 'FD-FAST-SS':
				subStyle = fdFastSS[style];
				break; 
			case 'FD-FAST-SL':
				subStyle = fdFastSL[style];
				break;
			case 'FD-LAX-RSS':
				subStyle = fdLaxRSS[style];
				break;
			case 'FD-LAX-SS':
				subStyle = fdLaxSS[style];
				break;
			case 'FD-LAXW-SS':
				subStyle = fdLaxWSS[style];
				break;
			case 'FD-SP-SS':
				subStyle = fdSpSS[style];
				break;
			case 'FD-SP-HD':
				subStyle = fdSpHD[style];
				break;
			case 'FD-SPW-SS':
				subStyle = fdSpwSS[style];
				break;
			case 'FD-SPW-HD':
				subStyle = fdSpwHD[style];
				break;
			case 'FD-SOC-SS':
				subStyle = fdSocSS[style];
				break;
			case 'FD-SOCW-SS':
				subStyle = fdSocWomensSS[style];
				break;
			default:
				subStyle = undefined;
				break;

		}

		log.l("End of getSubStyle function returning " + subStyle);
		return subStyle;
	}	

	//promptForgarmentCode Function Description
	//prompt the user for the garmentCode
	function promptForgarmentCode(name)
	{
		var result;

		var clickedCancel = false;
	
		var w = new Window("dialog","Enter the BM Code");
			var txtGroup = w.add("group");
				var topTxt = txtGroup.add("statictext", undefined, "Enter the BM Code for " + name);
		
			var inputGroup = w.add("group");
				var inputTxt = inputGroup.add("edittext",undefined,"eg. FD-163");

			var errMsgGroup = w.add("group");
		
			var btnGroup = w.add("group");
				var submit = btnGroup.add("button", undefined, "Submit");
					submit.onClick = function()
					{
						if(inputTxt.text.indexOf("eg.")>-1 || inputTxt.text == "")
						{
							var err = "You must enter a BM Code.";
							if(errMsgGroup.subGroup)
							{
								rmError(errMsgGroup);
							}
							displayError(err,errMsgGroup);
						}
						else
						{
							result = inputTxt.text;
							w.close();
						}
					}
				var cancel = btnGroup.add("button", undefined, "Cancel");
					cancel.onClick = function()
					{
						if(!clickedCancel)
						{
							log.l("User clicked cancel. Asking them to confirm cancel.");
							var err = "If you cancel, the " + name + " garment will be ignored.";
							if(errMsgGroup.subGroup)
							{
								rmError(errMsgGroup);
							}
							displayError(err,errMsgGroup);
							clickedCancel = true;
						}
						else
						{
							log.l("User confirmed cancellation. Skipping " + name);
							result = undefined;
							w.close();
						}
					}
		w.show();
		
		function rmError(eGroup)
		{
			var err = eGroup.subGroup.errorMsg;
			eGroup.remove(err.parent);
			w.layout.layout(true);
		}
		
		function displayError(msg,eGroup)
		{
			// errMsgGroup.remove(eGroup.parent);
			eGroup.subGroup = eGroup.add("group");
			eGroup.subGroup.errorMsg = eGroup.subGroup.add("statictext", undefined, msg);
			w.layout.layout(true);
		}
	
	
		return result;
	}

	log.l("End of getGarmentCode function. Returning: ::result.top = " + result.top + "::result.bottom = " + result.bottom);
	return result;
}